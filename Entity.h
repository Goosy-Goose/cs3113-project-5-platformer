...
enum AIState { AI_WALKING_L, AI_WALKING_R, AI_IDLING, ATTACKING, JUMPING };
enum AIAnimation { AI_IDLE, AI_RIGHT, AI_LEFT, AI_JUMP, AI_SHOOT };


enum PlayerAnimation { IDLE, RIGHT, LEFT, JUMP };
...
private:
    ...
    AIAnimation     m_ai_current_animation;
...
public:
    ...
    Entity(GLuint texture_id, float speed, float width, float height, EntityType EntityType, AIType AIType, AIState AIState, AIAnimation AIanim, std::vector<std::vector<int>> animations,
    ...
    void move_left() { m_movement.x = -1.0f; set_animation_state(LEFT);  }
    void move_right() { m_movement.x = 1.0f; set_animation_state(RIGHT); }
    ...
    int       const get_enemies_killed() const { return m_num_enemies_killed; }
    ...
    void set_animation_state(AIAnimation new_animation);
